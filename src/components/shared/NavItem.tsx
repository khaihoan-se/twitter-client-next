        ...
        <Link href={path}>
            <a className='flex items-center grow w-full'>
                <div className='flex items-center p-[12px] hover:bg-tt-hover-header-color rounded-full'>
                    <div>
                        {isActive(title === 'Profile' ? '/[username]' : path) ? <IconActive className='w-[1.75rem] h-[1.75rem]' /> : <Icon className='w-[1.75rem] h-[1.75rem]' />}
                    </div>
                    <div className={classNames(
                        'mr-[16px] ml-[20px] text-[20px] text-tt-text-color xl:block hidden',
                        isActive(title === 'Profile' ? '/[username]' : path) && 'font-bold'
                    )}>
                        <span>{title}</span>
                    </div>
                </div>
